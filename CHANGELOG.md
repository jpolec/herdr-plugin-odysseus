...
- **Model, effort and advisor per runner.** Runner config takes `model`, `effort` and `advisor`, translated to each agent's own flags (Claude `--model`/`--effort`/`--advisor`, Codex `-m`/`-c model_reasoning_effort=…`); `advisor: off` disables Claude's advisor for that runner whatever the user's Claude settings say. A named runner of a known agent kind needs no `mode` (`my-claude: {kind: claude, model: sonnet}`). Built-in presets `claude-deep` (opus, xhigh, advisor opus) and `claude-fast` (sonnet, medium, no advisor); the planner, conformance review and contract writer default to `claude-deep`. `runners` shows each runner's model, effort and advisor, and `eval run --runners claude-deep,claude,claude-fast` compares them on your own cases.
